...
check_directories() {
    ...
    if [ "$BACKUP_GITEA" = "true" ] && [ ! -d "$DATA_DIR" ]; then
        error "Директория данных Gitea не найдена: $DATA_DIR"
        return 1
    fi
    
    if [ "$BACKUP_POSTGRES" = "true" ] && [ ! -d "$POSTGRES_DATA_DIR" ]; then
...
create_backup() {
    ...
    cp -p ../.env "$BACKUP_PATH/" 2>/dev/null || warning "Файл .env не скопирован (отсутствует)"
...
main() {
    ...
    if ! check_directories; then
        error "Проверка директорий не пройдена"
        exit 1
    fi
    
    # Создаем бэкап
    if ! create_backup; then
        error "Создание бэкапа не удалось"
        exit 1
    fi
    
    # Архивируем
    if ! archive_backup; then
        error "Архивирование не удалось"
        exit 1
    fi
