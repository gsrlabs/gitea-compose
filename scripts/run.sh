...
show_help() {
    ...
    echo -e "    ${GREEN}backup${NC}    — Создать резервную копию (sudo)"
    echo -e "    ${GREEN}restore${NC}   — Восстановить из бэкапа (sudo)"
    echo -e "    ${GREEN}update${NC}    — Обновить до последней версии (sudo)"
    echo ""
